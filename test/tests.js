...
let $;
before(() => {
    $ = (sel) => frame.contentWindow.document.querySelector(sel);
});
const fireEvent = (node, eventName, path) => {
    let event;
    if (typeof testWindow().Event === 'function') {
        const MouseEvent = testWindow().MouseEvent;
        ...
        event.initEvent(eventName, true, true, this, 0, event.screenX, event.screenY, event.clientX, event.clientY, false, false, false, false, 0, null);
        event.button = 1;
        event.which = null;
    }
    if (path) {
